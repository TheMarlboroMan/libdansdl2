...
//!Channels can be monitored. A monitored channel does not call free() when SDL's callback is issued after the sound is played.
...
//!Will never work, only there to allow construction of audio_channel_safe
//!instances that will be linked later via assignment.
audio_channel_safe::audio_channel_safe() {
...
audio_channel_safe::audio_channel_safe(audio_channel& c)
	:channel(c.channel)
{
	if(!c.is_linked()) fail("construction");
}
...
{
	if(channel==nullptr) fail("resume");
	channel->resume();
}
...
{
	if(channel==nullptr) fail("fade_out");
	return channel->fade_out(ms);
}
...
{
	if(channel==nullptr) fail("is_looping");
	return channel->is_looping();
}
...
{
	if(channel==nullptr) fail("is_monitored");
	return channel->monitoring;
}
...
{
	if(channel==nullptr) fail("is_playing");
	return channel->playing;
}
...
{
	if(channel==nullptr) fail("is_paused");
	return channel->paused;
}
...
{
	if(channel==nullptr) fail("get_index");
	return channel->index;
}
...
{
	if(channel==nullptr) fail("get_repeat");
	return channel->repeat;
}
...
{
	if(channel==nullptr) fail("get_volume");
	return channel->volume;
}
...
{
	if(channel==nullptr) fail("set_volume");
	channel->set_volume(val);
}
...
{
	if(channel==nullptr) fail("set_monitoring");
	channel->set_monitoring(v);
}
...
{
	if(channel==nullptr) fail("assign_callback_listener");
	channel->assign_callback_listener(a);
}
...
{
	if(channel==nullptr) fail("clear_callback_listener");
	channel->clear_callback_listener();
}
...
{
	if(channel==nullptr) fail("set_stereo_volume");
	channel->set_stereo_volume(sp);
}
